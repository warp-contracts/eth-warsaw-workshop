...
import {
  ...
  deployAtomicNFT,
  AtomicNFTContract,
} from '../bindings/atomic-nft-js-binding';
...
describe('Testing the Atomic NFT Token', () => {
  ...
  it('should not transfer more than user balance', async () => {
    ...
    ).rejects.toThrow('Cannot create interaction: Caller balance not high enough to send 101 token(s)!');
  });
  ...
  it('should not transfer from more tokens than allowed', async () => {
    ...
    ).rejects.toThrow('Cannot create interaction: Caller allowance not high enough to send 21 token(s)!');
  });
